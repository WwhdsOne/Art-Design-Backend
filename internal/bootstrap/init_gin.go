...
func InitGin(m *middleware.Middlewares, logger *zap.Logger, c *config.Middleware) *gin.Engine {
	// 注册自定义校验器
	RegisterValidator()
	// 创建gin引擎
	engine := gin.New()
	...
	// 添加限流中间件
	engine.Use(m.RedisRateLimitMiddleware(c.RateLimit.Window, c.RateLimit.MaxReq))
	// 添加操作日志记录
	return engine
}
