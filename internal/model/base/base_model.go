...
type BaseModel struct {
	...
	CreatedAt time.Time `gorm:"type:timestamp with time zone;column:created_at;autoCreateTime"` // 创建时间字段，记录记录创建时的时间戳
	UpdatedAt time.Time `gorm:"type:timestamp with time zone;column:updated_at;autoUpdateTime"` // 更新时间字段，记录最后一次更新时的时间戳
