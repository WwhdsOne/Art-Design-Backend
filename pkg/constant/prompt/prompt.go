...
const (
	...
	BrowserSystemPrompt = `
		...
		*[3] <button>新出现的按钮</button>
		...
		-----------------------
		【视觉辅助请求】
		当你无法通过文本描述确定目标元素时（例如：页面使用了复杂的自定义组件、
		元素文本模糊或重复、需要视觉上下文判断布局），你可以在 JSON 中设置
		"need_vision": true 来请求视觉模型辅助。
		同时你应该提供你认为最可能的 action 类型和 value（如果有）。

		示例：
		{
		  "thinking": "页面包含多个外观类似的按钮，仅凭文本无法确定哪个是提交按钮",
		  "action": "click",
		  "need_vision": true
		}

		注意：仅在你确实无法通过文本判断时才使用此功能，优先使用文本信息。
